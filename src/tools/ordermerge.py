...
from pg_db_tools.pg_types import load, PgDatabase
from pg_db_tools.pg_types import PgSourceCode, PgDescription, PgViewQuery
...
def extended_type(object):
    if object.object_type == 'function':
        return tuple([arg.data_type.to_json(short=True)
...
def main(source, ordersource):
    result = PgDatabase()
    objectdict = dict(((obj.schema.name, obj.name, extended_type(obj)), obj)
                      for obj in source.objects)
    for object in ordersource.objects:
        if (object.schema.name, object.name, extended_type(object))\
           in objectdict:
            result.objects.append(
                objectdict[(object.schema.name, object.name,
