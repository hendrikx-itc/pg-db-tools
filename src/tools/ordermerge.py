...
Result: All elements in the form they are in file1, but in the order from file2.
  All elements from file1 that are not in file2 are put at the end.
"""

from pg_db_tools.pg_types import load, PgDatabase, PgSourceCode, PgDescription, PgViewQuery
from pg_db_tools.sql_renderer import SqlRenderer
from pg_db_tools.commands.extract_from_db import format_yaml
import sys

def extended_type(object):
...
def main(source, ordersource):
    result = PgDatabase()
    objectdict = dict(((obj.schema.name, obj.name, extended_type(obj)), obj) for obj in source.objects)
    for object in ordersource.objects:
        if (object.schema.name, object.name, extended_type(object)) in objectdict:
            result.objects.append(objectdict[(object.schema.name, object.name, extended_type(object))])
